...
int main(int argc, const char * argv[])
{
    ...
    int length = read_names(names, "names.txt"); //found below
    if (length != FAILURE)
    {
        shuffle_names(names, length); //found below
        //For debugging
    ...
    while (!game_over(list)) //found in assassin_list.c must create
    {
        one_game(list); //found below
    }
...
void one_game(alist list)
{
    ...
    printf("\nnext victim? ");
    gets(victim);
    if (grave_yard_contains(list, victim)) //found in assassin_list.c must create
    {
        printf("%s is already dead.\n", victim);
    }
    else if (!kill_ring_contains(list, victim)) //found in assassin_list.c must create
    {
        puts("Unknown person");
    }
    else
    {
        kill_victim(list, victim); //found in assassin_list.c must create
    }
}
